...
async def check_activity_limit(
    ...
    if display_data_cleared:
        ...
        async with db.pool.acquire() as conn:
            ...
            """, chat_id, uid, today, act)
            current_count = row["activity_count"] if row else 0
    else:
        # 未软重置：从原始表读取
...
async def start_activity(message: types.Message, act: str):
    ...
    async with user_lock:
        ...
        logger.info(
            f"📝 用户 {uid} 开始活动 {act}，消息ID: {sent_message.message_id}"
        )
...
async def cmd_fix_message_refs(message: types.Message):
    ...
    try:
        ...
        logger.info(f"管理员 {message.from_user.id} 清除了群组 {chat_id} 的 {updated_count} 个消息引用")
...
async def show_history(message: types.Message):
    ...
    last_soft_reset = user_data.get("last_soft_reset")
    display_data_cleared = user_data.get("display_data_cleared", False)

    if display_data_cleared:
        text += "🔄 当前周期已执行软重置（以下仅显示软重置后的记录）\n"

    text += "📊 当前周期记录：\n\n"

    activity_limits = await db.get_activity_limits_cached()

    source_table = "user_activities_display" if display_data_cleared else "user_activities"
    ...
    if last_soft_reset:
        text += f"\n🧽 最近软重置时间：<code>{last_soft_reset.strftime('%H:%M')}</code>\n"
...
async def show_rank(self, message: types.Message):
    """显示排行榜 - 支持软重置和原始数据展示"""
    chat_id = message.chat.id
    uid = message.from_user.id
    ...
    # 根据软重置状态选择数据源
    table_name = "user_activities_display" if display_data_cleared else "user_activities"
            ...
            for chat_id in all_groups:
                try:
                    ...
                    if now.hour == reset_hour and last_reset_hour != now.hour:
                        logger.info(f"⏰ 群组 {chat_id} 到达重置小时 {reset_hour}，启动重置流程")
            ...
            try:
                ...
                if now.hour == soft_hour and now.minute == soft_minute:
                    ...
                    for user_data in group_members:
                        ...
                        async with user_lock:
                            success = await db.reset_user_soft_daily_data(chat_id, user_data["user_id"])
                            if success:
                                reset_count += 1
