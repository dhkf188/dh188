...
async def can_perform_activities(
    ...
    if now - shift_start_time > timedelta(hours=16):
        await db.clear_shift_state(chat_id)
        return False, "❌ 班次已过期（超过16小时），请重新上班打卡！"
    ...
    if shift_info is None or shift_info.get("shift_detail") is None:
        ...
        return False, (
            f"❌ 当前时间不在活动允许的时间窗口内\n"
            f"📊 白班活动时间：<code>{day_start} ~ {day_end}</code>\n"
            f"📊 夜班活动时间：<code>{day_end} ~ 次日 {day_start}</code>\n\n"
    ...
    business_date = shift_info.get("business_date", frozen_date)

    # 如果班次状态与班次判定的日期不一致，记录日志但继续执行
    if frozen_date != business_date:
        logger.warning(
            f"⚠️ [活动检查] 日期不一致: 班次状态日期={frozen_date}, "
            f"班次判定日期={business_date}, 使用班次状态日期"
        )

    logger.info(
        f"📊 [活动检查] 窗口验证通过: shift_detail={shift_detail}, "
...
async def start_activity(message: types.Message, act: str):
    ...
    async with user_lock:
        ...
        if shift_info is None:
            logger.error(
                f"[开始活动] determine_shift_for_time 返回 None，使用班次状态作为 fallback"
            )
            current_shift = shift_state["current_shift"]
            record_date = shift_state["record_date"]

            # 手动计算 shift_detail（作为 fallback）
            ...
            current_shift = shift_info["shift"]  # "day" 或 "night"
            shift_detail = shift_info[
                "shift_detail"
            ]  # "day", "night_last", "night_tonight"
            record_date = shift_info["record_date"]

            # 验证记录日期是否与班次状态的日期一致（日志记录，不阻止流程）
            if record_date != shift_state["record_date"]:
                logger.warning(
                    f"⚠️ [开始活动] 日期不一致: 班次状态日期={shift_state['record_date']}, "
                    f"班次判定日期={record_date}, 使用班次判定日期"
                )
...
async def process_work_checkin(message: types.Message, checkin_type: str):
    ...
    async with user_lock:
        ...
        shift_info = await db.determine_shift_for_time(
            ...
            active_shift=shift_state["current_shift"] if shift_state else None,
        )
