...
async def can_perform_activities(
    ...
    if not shift_state:
        return False, "❌ 当前没有活跃的班次，请先打上班卡！"

    if shift_state["active_worker_count"] <= 0:
        # 清理无效状态
        await db.clear_shift_state(chat_id)
        return False, "❌ 当前班次已结束，请重新打卡！"

    active_shift = shift_state["current_shift"]
    frozen_date = shift_state["record_date"]

    logger.info(
        f"📊 [活动检查] 当前班次: {active_shift}, "
        f"人数={shift_state['active_worker_count']}, 冻结日期={frozen_date}"
    )
    ...
    async with db.pool.acquire() as conn:
        ...
        if not has_work_start:
            shift_text = "白班" if active_shift == "day" else "夜班"
...
async def start_activity(message: types.Message, act: str):
    ...
    async with user_lock:
        ...
        # 检查班次是否有效（人数>0）
        if shift_state["active_worker_count"] <= 0:
            await db.clear_shift_state(chat_id)  # 清理无效状态
            await message.answer(
                "❌ 当前班次已结束，请重新打卡！",
                reply_markup=await get_main_keyboard(
                    chat_id=chat_id, show_admin=await is_admin(uid)
        ...
        logger.info(
            f"🔄 [开始活动] 使用群组班次状态: shift={current_shift}, "
            f"detail={shift_detail}, record_date={record_date}, "
            f"人数={shift_state['active_worker_count']}"
        )
        ...
        logger.info(
            ...
            f"记录日期: {record_date}, 班次状态人数: {shift_state['active_worker_count']}"
        )
...
async def process_work_checkin(message: types.Message, checkin_type: str):
    ...
    async with user_lock:
        ...
        # ========== 获取业务日期和班次信息 ==========
        if is_dual_mode and shift_state:
            # 双班模式且有活跃班次：使用班次状态中的数据
        ...
        if checkin_type == "work_start":
                        ...
                        async with conn.transaction():
                            ...
                            if is_dual_mode:
                                ...
                                current_state = await conn.fetchrow(
                                    """
                                    SELECT current_shift, active_worker_count 
                                ...
                                if (
                                    current_state
                                    and current_state["current_shift"] != shift
                                ):
                                    logger.warning(
                                        f"⚠️ [{trace_id}] 班次冲突检测: "
                                        f"当前={current_state['current_shift']}, 新={shift}"
                                    )
            ...
            await send_work_notification(
                ...
                expected_dt,
                action_text,
                status_type if is_late_early else "准时",
                fine_amount,
                trace_id,
            )
                    ...
                    async with db.pool.acquire() as conn:
                        async with conn.transaction():
                            ...
                            if is_dual_mode:
                                ...
                                logger.info(
                                    f"📊 [{trace_id}] 下班后剩余人数: {current_count}"
                                )
                                ...
                                if current_count <= 0:
                                    ...
                                    if remaining_users:
                                        ...
                                        logger.info(
                                            ...
                                            f"{'...' if len(remaining_users) > 3 else ''}"
                                        )
        ...
        if shift == "day":
            ...
            async with db.pool.acquire() as conn:
                ...
                if now < today_start:
                    ...
                    if checkin_type in last_night:
                        target_start = last_night[checkin_type]["start"]
                        target_end = last_night[checkin_type]["end"]
                else:
                    # 其他时间，用今晚夜班窗口
                    tonight = night_window.get("tonight", {})
                    if checkin_type in tonight:
                        target_start = tonight[checkin_type]["start"]
...
async def _get_existing_work_record(
    ...
    try:
        ...
        # 复用 calculate_shift_window 获取窗口信息
        window_info = db.calculate_shift_window(
            shift_config=shift_config, checkin_type=checkin_type, now=now
        )
        ...
        if shift == "day":
            if checkin_type in window_info.get("day_window", {}):
                target_start = window_info["day_window"][checkin_type]["start"]
                target_end = window_info["day_window"][checkin_type]["end"]
        else:
            current_shift = window_info.get("current_shift")
            night_window = window_info.get("night_window", {})

            if current_shift == "night_last":
                last_night = night_window.get("last_night", {})
                if checkin_type in last_night:
                    target_start = last_night[checkin_type]["start"]
                    target_end = last_night[checkin_type]["end"]
            elif current_shift == "night_tonight":
                tonight = night_window.get("tonight", {})
                if checkin_type in tonight:
                    target_start = tonight[checkin_type]["start"]
                    target_end = tonight[checkin_type]["end"]
        ...
        if not target_start or not target_end:
            logger.warning(
                f"无法从窗口确定打卡范围，使用24小时fallback: {shift} {checkin_type}"
            )
        ...
        async with sem:
            try:
                ...
                if is_dual_mode:
                    ...
                    mode_info = f"双班模式(执行时间:{execute_time.strftime('%H:%M')},{period_info})"
                    ...
                    # 单班模式幂等性检查
                    reset_flag_key = f"single_reset:{chat_id}:{now.strftime('%Y%m%d')}"
                    if global_cache.get(reset_flag_key):
                        logger.debug(f"⏭️ 群组 {chat_id} 今天已执行单班重置")
                        return
        ...
        try:
            ...
            if now.hour == Config.CLEANUP_HOUR and now.minute == Config.CLEANUP_MINUTE:
                if Config.AUTO_CLEANUP_ENABLED:
                    logger.info(
                        f"🧹 开始自动清理 ({Config.DATA_RETENTION_DAYS}天前的数据)"
                    )
            ...
            if (
                ...
                if Config.MONTHLY_EXPORT_ENABLED:
                    ...
                    logger.info(
                        f"📊 月度导出完成: {success_count}/{len(all_groups)} 个群组"
                    )
