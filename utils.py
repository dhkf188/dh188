...
# ========== 批量配置获取函数 ==========
async def get_batch_config(chat_id: int, uid: int, activity: str = None):
    """
    批量获取配置 - 减少数据库查询次数
    返回包含所有必要配置的字典
    """
    cache_key = f"batch_config:{chat_id}:{uid}:{activity if activity else 'none'}"
    ...
    # 群组配置
    tasks.append(db.get_group_cached(chat_id))

    # 班次配置
    tasks.append(db.get_shift_config(chat_id))

    # 活动配置（如果提供了活动名）
    if activity:
        tasks.append(db.get_activity_limits_cached())
        tasks.append(db.get_activity_time_limit(activity))
        tasks.append(db.get_activity_max_times(activity))
        tasks.append(db.get_activity_user_limit(activity))
        tasks.append(db.get_fine_rates_for_activity(activity))

    # 用户数据
    tasks.append(db.get_user_cached(chat_id, uid))

    # 用户活跃班次
    tasks.append(db.get_user_active_shift(chat_id, uid))

    # 推送设置
    tasks.append(db.get_push_settings())
    ...
    config = {
        ...
        "shift_config": (
            ...
            if not isinstance(results[result_idx + 1], Exception)
            else {}
        ),
    }
    ...
    if activity:
        ...
        config["max_times"] = (
            results[result_idx + 2]
            if not isinstance(results[result_idx + 2], Exception)
            else 0
        )
        config["user_limit"] = (
            results[result_idx + 3]
            if not isinstance(results[result_idx + 3], Exception)
            else 0
        )
        config["fine_rates"] = (
            results[result_idx + 4]
            if not isinstance(results[result_idx + 4], Exception)
            else {}
        )
        result_idx += 5

    config["user_data"] = (
        results[result_idx] if not isinstance(results[result_idx], Exception) else None
    )
    config["active_shift"] = (
        results[result_idx + 1]
        if not isinstance(results[result_idx + 1], Exception)
    ...
    # 缓存5秒
    global_cache.set(cache_key, config, ttl=5)
    return config
